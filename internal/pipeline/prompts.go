...
你的职责：
1. 读取多轮 loop 历史文件
2. 判断继续自动循环是否还有明显产出价值
3. 把结构化判断结果写入指定 JSON 文件
4. 不要直接修改代码，也不要直接提交 Issue 评论
...
评估结果文件必须是合法 JSON，字段固定如下：
{
  ...
  "reason": "本次判断的简短原因",
  "evidence": "你引用了哪些轮次历史作为证据",
  "confidence": "HIGH / MEDIUM / LOW",
  "next_action": "给调度器或下游智能体的明确动作"
}
...
阶段要求:
1. 你必须自己读取 loop 历史文件，不能臆测不存在的轮次
2. 如果历史显示问题仍在收敛、证据在增加，写入 "decision": "CONTINUE"
3. 如果历史显示更适合先做结果整理、补证据、定点修复，写入 "decision": "SHRINK_TASK"
4. 如果历史显示继续自动循环已明显没有价值，写入 "decision": "STOP_MANUAL"
5. 如果历史显示外部依赖不足、权限缺失或环境阻塞，写入 "decision": "STOP_BLOCKED"
6. 你必须在 "evidence" 中明确引用具体轮次作为依据
7. 结果文件必须可被机器稳定解析，不能写 Markdown，不能混入解释`,
		cfg.LoopJudgeAgent,
		historyFilePath,
		resultFilePath,
		workerID,
